...
---
## What it looks like?
Here, the front end is sending data from a GET request (old params) and a PUT request (updated payload/JSON).
### Initial payload
```js
{
...
### The complete flow - as discussed with Purushottam sir
```txt
- if you check the data model working behind the scenes for service job updation or any kind of updation, we see common patterns. There is one backend service that receives an api, api has an endpoint containing what to update like service names, we will identify these apis, group them iff needed. when backend server receives a request or input payload, it simply saves those asked changes at some entity so that the data is not directly gets persisited and needs a human approval/system approval to see those changes in effect, and for this purpose i designed new entity name(s): "ServiceJobEditAppl".
This entity will save information about the person who is requesting change due to some project/task need, the datetime stamp when the changes were applied. Mention fields clearly: [applicationdata,action,actionType, applicationDateTime, createdDateTime, statusId(approved,pending,rejected),requestedByUserId,ApprovedByUserId] . This changed payload/updations are stored directly as a JSON in this above addon entity. after this when approver gets the email that a change is requested or when reviewer is assigned and reviewer opens a dashboard where they see all critical entities with fields were a change is requested by an active user. The action type will point to an html artifact, these artificat will call 2 apis/or get data from framework level ---and will display that data on the template screen designed for this purpose. Clearly this UI dashboard or email screen will contain entities where there are several services and their data needs to be updated. The screen will show 2 json and below that their diff too, and reviewer will have the access of revieing changes When a user clicks the submit button/approve button the backend logic will run like- it will retrieve all i/p data and action (gets this by Application_Request_Id field which is PK of that addon table. ). After getting  data and action, it calls a wrapper service approve#request. It has data and action and approve#request will  now  call main service(that does the db hit for persisiting changes)  - here service name is update#sreviceJob. 
note: the request is not limited to service jobs, it could be any entity and different nesting levels. Finally after all these operations the data is finally persisted and table ServiceJobEditAppl status will get updated after filtering.  
```
